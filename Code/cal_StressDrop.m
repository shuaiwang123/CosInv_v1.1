% function [Mo Mw] = cal_StressDrop(Len,wid,slip)
function StressDrop = cal_StressDrop(Len,Wid,slip)
...
shear_modulus = 30.*GPa2Pa;                   % shear modulus

area = Len*Wid;
area = area*km2m^2;
Mo = shear_modulus * area.*slip;
Mw = 2/3 * log10(Mo) - 6.06;

StressDrop = 2*Mo/(pi*(Len*km2m)*(Wid*km2m)^2);  % Pa
StressDrop = 10^-6*StressDrop;          % MPa
